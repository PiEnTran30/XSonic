...
import { Wallet, Transaction, Plan, Voucher } from "../../types/database";
...
export class BillingAdapter {
  ...
  async applyVoucher(userId: string, code: string): Promise<number> {
    ...
    if (now < validFrom) throw new Error("Voucher not yet valid");
    if (validUntil && now > validUntil) throw new Error("Voucher expired");
    ...
    if (usage) throw new Error("Voucher already used");
